...
public class WeftPick {
    ...
    /**
     * Sets the given treadle to be selected, clearing any other selections. It
     * is used for non-liftplan drafts, and also for restoring drafts from older
     * persisted versions where the property was called treadleId.
     *
    ...
    public void setTreadleId(int i) {
        ...
        if (i == -1 || !treadles[i]) {
            ...
            if (i != -1) {
                treadles[i] = true;
            } else if (treadles != null) {
                for (int j = 0; j < treadles.length; j++) {
                    treadles[j] = false;
                }
            }
    ...
    public void validate(int treadles) throws IllegalArgumentException {
        if (this.treadles.length != treadles) {
            throw new IllegalArgumentException();
        }
    }
