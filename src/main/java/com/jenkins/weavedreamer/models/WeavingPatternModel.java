...
public class WeavingPatternModel extends AbstractWeavingDraftModel {
        ...
        setDraftListener(ev -> {
            ...
            if (propName.equals("ends")) {
                if (iev == null) {
                    ...
                    if (iev.getIndex() + 1 != draft.getEnds().size()) {
                        fireTableColumnUpdated(iev.getIndex() + 1);
                    }
                }
    ...
    public void fireTableColumnUpdated(int column) {
        fireTableChanged(new TableModelEvent(this,
                0, getRowCount() - 1, getColumnCount() - 1 - column));
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        final boolean isWarp = draft.isWarpVisible(endId, rowIndex);
        final boolean warpFloat = isWarp // next row down
                && rowIndex < draft.getPicks().size() - 1
                && draft.isWarpVisible(endId, rowIndex + 1);
        final boolean weftFloat = !isWarp
                && endId > 0
                && !draft.isWarpVisible(endId - 1, rowIndex);

        return new WeavingPatternCellModel(
                draft.getVisibleColor(endId, rowIndex),
                !weftFloat,
                !warpFloat);
    }
